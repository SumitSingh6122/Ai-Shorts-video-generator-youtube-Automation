...
interface VideoData {
  audioURL: string;
  captions: Array<{ start: number; end: number; word: string }>;
  images: string[];
  captionStyle: {
    color: string;
    fontSize: string;
...
const VideoGenerator=()=> {
  ...
  const startRender = async (data: VideoData) => {
    setStatus('loading');
    setError(null);
    setVideoUrl(null);
  ...
  const pollStatus = async (checkUrl: string) => {
    try {
      ...
      if (status === 'completed') {
        ...
        setTimeout(() => pollStatus(checkUrl), 5000);
      }
    } catch (err) {
      setStatus('error');
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      {/* Your form UI here */}
      <button 
        onClick={() => startRender(sampleData)}
        disabled={status === 'loading'}
        className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 disabled:bg-gray-400"
      >
        {status === 'loading' ? 'Generating...' : 'Create Video'}
      </button>
